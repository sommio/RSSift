...
Ask the user the following questions (using the platform's blocking question tool — e.g., `ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini — or present numbered options and wait for a reply):
