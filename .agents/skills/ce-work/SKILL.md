...
- `ce:work` is allowed to consume, interpret, and update existing plans, including checkbox progress and `status` fields. It must NEVER create a new plan file inside `docs/en/plans/` or `docs/zh-Hans/plans/`. This guardrail also applies to subagents and wrap-up notes: do not create new plan files to record execution progress or to backfill missing language variants.
- If no existing plan is available and the work needs one, stop and route the user through `/ce:plan` rather than authoring a new plan file from `ce:work`.
- If only one language side of a paired plan exists, you may continue from that existing plan, but do not create the missing counterpart from `ce:work`; refer the user to `/ce:plan` instead.
