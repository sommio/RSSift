...
- **Never write inline scripts** (`python3 -c`, `node -e`) to process, filter, sort, or transform issue data. Reason over the data directly after reading it — you are an LLM, you can filter and cluster in context without running code.
