...
If no issue number or URL was provided as an argument, ask the user for one before proceeding (using the platform's question tool -- e.g., `ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini -- or present a prompt and wait for a reply).
...
2. Ask the user (using the platform's question tool -- e.g., `ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini -- or present options and wait for a reply) whether they can set up the required conditions
