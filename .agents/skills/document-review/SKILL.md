...
- `present` findings are returned as structured text for the caller to handle -- no ask_user_question prompts, no interactive approval
...
```
Skill("compound-engineering:document-review", "mode:headless docs/zh-Hans/plans/my-plan.md")  # also load the paired docs/en/plans/my-plan.md when present
```
...
**If no document is specified (interactive mode):** Ask which document to review, or find the most recent document in the bilingual brainstorm or plan trees using a file-search/glob tool (e.g., Glob in Claude Code).
...
- **requirements** -- from the paired brainstorm directories `docs/en/brainstorms/` and `docs/zh-Hans/brainstorms/`, focuses on what to build and why
- **plan** -- from the paired plan directories `docs/en/plans/` and `docs/zh-Hans/plans/`, focuses on how to build it with implementation details
