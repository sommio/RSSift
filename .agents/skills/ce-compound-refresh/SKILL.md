...
description: Refresh stale or drifting learnings and pattern docs in `docs/en/solutions/` and `docs/zh-Hans/solutions/` by reviewing, updating, consolidating, replacing, or deleting them against the current codebase. Use after refactors, migrations, dependency upgrades, or when a retrieved learning feels outdated or wrong. Also use when reviewing `docs/en/solutions/` and `docs/zh-Hans/solutions/` for accuracy, when a recently solved problem contradicts an existing learning, when pattern docs no longer reflect current code, or when multiple docs seem to cover the same topic and might benefit from consolidation.
disable-model-invocation: true
---

# Compound Refresh

Maintain the quality of `docs/en/solutions/` and `docs/zh-Hans/solutions/` over time. This workflow reviews existing learnings against the current codebase, then refreshes any derived pattern docs that depend on them.
...
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. Git history preserves every deleted file — that is the archive. A dedicated archive directory creates problems: archived docs accumulate, pollute search results, and nobody reads them. If someone needs a deleted doc, `git log --diff-filter=D -- docs/en/solutions/ docs/zh-Hans/solutions/` will find it.

## Scope Selection

Start by discovering learnings and pattern docs under `docs/en/solutions/` and `docs/zh-Hans/solutions/`.

Exclude:

- `README.md`
- `docs/en/solutions/_archived/` or `docs/zh-Hans/solutions/_archived/` (legacy — if this directory exists, flag it for cleanup in the report)

Find all `.md` files under `docs/en/solutions/` and `docs/zh-Hans/solutions/`, excluding `README.md` files and anything under `_archived/`. If an `_archived/` directory exists, note it in the report as a legacy artifact that should be cleaned up (files either restored or deleted).
...
1. **Directory match** — check if the argument matches a subdirectory name under `docs/en/solutions/` and `docs/zh-Hans/solutions/` (e.g., `performance-issues`, `database-issues`)
...
```text
No candidate docs found in `docs/en/solutions/` and `docs/zh-Hans/solutions/`.
...
After reviewing the underlying learning docs, investigate any relevant pattern docs under ``docs/en/solutions/patterns/` and `docs/zh-Hans/solutions/patterns/``.
...
**Legacy cleanup** (if `docs/en/solutions/_archived/` or `docs/zh-Hans/solutions/_archived/` exists):
...
2. Assess whether an agent reading the instruction files would learn three things:
   ...
   This is a semantic assessment, not a string match. The information could be a line in an architecture section, a bullet in a gotchas section, spread across multiple places, or expressed without ever using the exact path `docs/en/solutions/` and `docs/zh-Hans/solutions/`. Use judgment — if an agent would reasonably discover and use the knowledge store after reading the file, the check passes.
