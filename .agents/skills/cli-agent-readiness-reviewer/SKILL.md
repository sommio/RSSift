...
- Argument parsing libraries: Click, argparse, Commander, cac, clap, Cobra, yargs, oclif
...
### TypeScript — pnpm workspace CLI

**Gives you for free:**
- `pnpm exec <cli>` and workspace scripts give a consistent way to invoke the CLI in dev, CI, and monorepos
- `package.json` `bin` wiring works cleanly with TypeScript CLIs once they are built or run through `tsx`
- Workspace filters such as `pnpm --filter <pkg> exec <cli>` make package-scoped automation explicit

**Doesn't give you — must implement:**
- `--json` or `--format` output remains a command concern even when the package is launched via `pnpm exec`
- Stdin handling is still manual — read `process.stdin` or accept `-` as a filename alias where appropriate
- TTY-aware defaults still require `process.stdout.isTTY` / `process.stdin.isTTY` checks in the CLI itself
- Exit codes and stdout/stderr separation still depend on the command handler implementation

**Anti-patterns to flag:**
- Documentation that only shows `pnpm run` examples when the workspace standard favors direct `pnpm exec` usage
- CLI usage that requires `pnpm run <script>` wrappers for every command instead of supporting direct `pnpm exec <cli>` invocation
- Assuming the package manager handles machine-readable output, retries, or prompt suppression for the CLI
