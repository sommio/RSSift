...
// TypeScript stack review
Task({
  subagent_type: "compound-engineering:review:kieran-typescript-reviewer",
  description: "TypeScript review",
  prompt: "Review this React Router + Hono TypeScript code for type safety and maintainability"
})
...
- `julik-frontend-races-reviewer` - React async UI race conditions
...
Task({
  ...
  prompt: "Research current best practices for JWT authentication with Hono and React Router in 2024-2026"
})

// Framework documentation
Task({
  subagent_type: "compound-engineering:research:framework-docs-researcher",
  description: "Research Drizzle migrations",
  prompt: "Gather comprehensive documentation about Drizzle ORM migrations and schema workflows"
})
...
```json
{
  ...
  "permissionSuggestions": ["Bash(pnpm *)"],
  "createdAt": 1706000000000
}
```
...
const research = await Task({
  ...
  prompt: "Research best practices for implementing caching in Hono APIs. Include: cache invalidation strategies, Redis vs in-memory caches, cache key design, and how React Router data loading changes cache boundaries."
})
...
Task({
  ...
  prompt: `
    ...
    Focus on the Hono user routes in server/routes/users.ts.
  `
})
```
